...
class RealEstateProjectFieldLabel(Base):
    """A per-company display name (and, for lists, the allowed options) for one of
    a RealEstateProject's configurable fields. `kind` is
    'param' | 'number' | 'flag' | 'list' and `idx` is the 1-based index within
    that kind. Only customized fields are stored; the rest fall back to their
    default label with no options."""

    __tablename__ = "realestate_project_field_labels"
    __table_args__ = (
        UniqueConstraint("company_id", "kind", "idx", name="uq_company_re_project_field"),
        Index("ix_re_project_field_label_company", "company_id"),
    )
...
def default_re_project_label(kind: str, idx: int) -> str:
