...
# Counts of the fixed configurable real-estate-project fields (labels per-company).
RE_PROJECT_PARAM_COUNT = 15   # free-text params
RE_PROJECT_NUM_COUNT = 15     # numeric params
RE_PROJECT_FLAG_COUNT = 5     # yes/no flags
RE_PROJECT_LIST_COUNT = 10    # single-choice lists


class RealEstateProject(Base):
    """A company real-estate project (tenant-scoped). Keeps a small set of BASIC
    fields, then a flexible block of configurable fields — text params, numeric
    params, yes/no flags and single-choice lists — mirroring the project model.
    The per-company display names (and list options) live on
    RealEstateProjectFieldLabel."""

    __tablename__ = "realestate_projects"
    __table_args__ = (
        Index("ix_realestate_projects_company", "company_id"),
    )
    ...
    # ---- configurable fields (labels per-company on RealEstateProjectFieldLabel) ----
