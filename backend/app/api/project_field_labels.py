"""Per-company display names (and list options) for a Project's configurable
fields: params / numbers / flags / lists.
...
from ..schemas.project_field_label import (
    ProjectFieldLabelOut,
    ProjectFieldLabelsUpdate,
    ProjectFieldOrderUpdate,
)


router = APIRouter(prefix="/api/project-field-labels", tags=["project-field-labels"])
...
_COUNTS = {
    "param": PROJECT_PARAM_COUNT,
    "number": PROJECT_NUM_COUNT,
    "flag": PROJECT_FLAG_COUNT,
    "list": PROJECT_LIST_COUNT,
}
...
def _resolve(db: Session, company_id: int) -> list[ProjectFieldLabelOut]:
    stored = {
        (r.kind, r.idx): r
        for r in db.query(ProjectFieldLabel).filter(ProjectFieldLabel.company_id == company_id)
    }
    out: list[ProjectFieldLabelOut] = []
        ...
        for i in range(1, count + 1):
            ...
            out.append(
                ProjectFieldLabelOut(
                    kind=kind,
                    idx=i,
                    label=(row.label if row else None) or default_project_label(kind, i),
...
@router.get("", response_model=list[ProjectFieldLabelOut])
def list_labels(
...
@router.put("", response_model=list[ProjectFieldLabelOut])
def update_labels(
    body: ProjectFieldLabelsUpdate,
    db: Session = Depends(get_db),
    ...
    for item in body.labels:
        ...
        is_default = not label or label == default_project_label(item.kind, item.idx)
        ...
        if is_default and not options and item.is_active and not has_custom_order:
            if row:
                db.delete(row)
            continue
        resolved = label or default_project_label(item.kind, item.idx)
        if row:
            row.label = resolved
            row.options = options
...
@router.put("/order", response_model=list[ProjectFieldLabelOut])
def update_order(
    body: ProjectFieldOrderUpdate,
    db: Session = Depends(get_db),
    _: User = Depends(require_company_admin),
    company_id: int = Depends(resolve_company_id),
):
    """Persist a custom global display order for the company's project fields.
    The body lists every field in the desired order; position i becomes the
    field's sort_order. Rows are created on demand to hold the order."""
    existing = {
        (r.kind, r.idx): r
        for r in db.query(ProjectFieldLabel).filter(ProjectFieldLabel.company_id == company_id)
    }
        ...
        if row:
            ...
            db.add(
                ProjectFieldLabel(
                    ...
                    kind=ref.kind,
                    idx=ref.idx,
                    label=default_project_label(ref.kind, ref.idx),
                    options=[],
                    is_active=True,
                    sort_order=position,
                )
