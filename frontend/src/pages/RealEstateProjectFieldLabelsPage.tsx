...
import { RealEstateProjectFieldLabels, type RealEstateProjectFieldKind } from '../lib/api'
import { Field, inputStyle } from '../components/Modal'
import { useAuth, useEffectiveCompanyId } from '../lib/AuthContext'

type Item = { kind: RealEstateProjectFieldKind; idx: number; label: string; optionsText: string; is_active: boolean }

const SECTIONS: { kind: RealEstateProjectFieldKind; title: string }[] = [
...
/** Rename a company's real-estate project fields (and set the options for list fields). */
export default function RealEstateProjectFieldLabelsPage() {
  const { user } = useAuth()
  ...
  function patch(kind: RealEstateProjectFieldKind, idx: number, p: Partial<Item>) {
    setSaved(false)
  ...
  async function save() {
    ...
    try {
      ...
      await RealEstateProjectFieldLabels.update(labels, cid)
      setSaved(true)
    } catch (e) {
      setError(String(e))
    } finally {
  ...
  const of = (kind: RealEstateProjectFieldKind) => items.filter((it) => it.kind === kind)
