...
import { RealEstateProjectFieldLabels, type RealEstateProjectFieldLabel, type RealEstateProjectFieldKind } from '../lib/api'
...
const KIND_BADGE: Record<RealEstateProjectFieldKind, string> = {
  param: 'פרמטר',
  number: 'מספר',
  flag: 'דגל',
  list: 'רשימה',
}
const KIND_COLOR: Record<RealEstateProjectFieldKind, string> = {
  param: '#1f3a5f',
  number: '#0f766e',
  flag: '#b45309',
  list: '#6d28d9',
}
const keyOf = (f: { kind: RealEstateProjectFieldKind; idx: number }) => `${f.kind}:${f.idx}`
...
export default function RealEstateProjectFieldOrderPage() {
  ...
  async function save() {
    ...
    setSaved(false)
    try {
      await RealEstateProjectFieldLabels.reorder(items.map((f) => ({ kind: f.kind, idx: f.idx })), cid)
      setSaved(true)
    } catch (e) {
      setError(String(e))
    } finally {
